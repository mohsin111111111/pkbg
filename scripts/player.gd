...
enum WeaponType {LASER, SHOTGUN, SNIPER,RIFLE}
...
var weapon_db = {
	...
	WeaponType.RIFLE: {"damage": 18, "rate":0.09, "rays":1, "spread":0.04, "range": -150.0}
}
...
var is_rappelling = false
const RAPPEL_SPEED = -10
var is_on_ladder = false
var ladder_speed = 4.0 

@export var drone_prefab: PackedScene 
var active_drone = null
var is_driving_drone = false
const BASE_FOV = 75.0
const AIM_FOV = 40.0
...
@onready var instruction_text = $HUD/InstructionText
...
func _unhandled_input(event):
	if event.is_action_pressed("weapon_1"): 
		current_weapon = WeaponType.LASER
		anim_player.stop()
...
func _physics_process(delta):
	if is_on_ladder:
		...
		elif Input.is_action_pressed("move_backward") or Input.is_action_pressed("ui_down"):
			velocity.y = -ladder_speed
	...
	if is_rappelling:
		velocity.x = 0
		velocity.z = 0
		velocity.y = RAPPEL_SPEED 
		move_and_slide()
		if is_on_floor() or Input.is_action_just_pressed("ui_accept"):
			is_rappelling = false
			print("Touched down safely!")
		return 
...
func reload_weapon():
	...
	var tween = get_tree().create_tween()
	tween.tween_property(reload_bar, "value", 100.0, 1.5)
...
func try_interact():
	...
	if raycast.is_colliding():
		...
		if target.has_method("get_rappelled"):
			is_rappelling = true
			global_position.x = target.global_position.x
			global_position.z = target.global_position.z
			print("Grabbed the rope! Sliding down...")
			
		elif target.has_method("rescue"):
			target.rescue()
			add_score(100) 
	...
	if raycast.is_colliding():
		...
		if target.name == "StealthGuard":
			...
			if disguise_prefab:
				...
				uniform.global_position = global_position
				var forward_direction = -global_transform.basis.z
				uniform.global_position += forward_direction * 1.5
...
func toggle_drone():
	if active_drone == null:
		if not drone_prefab:
			print("Error: Please drop rc_drone.tscn into the Player Inspector!")
			return
		print("Deploying Drone!")
		active_drone = drone_prefab.instantiate()
		get_tree().current_scene.add_child(active_drone)
		var drop_pos = global_position - (global_transform.basis.z * 2.0)
		drop_pos.y -= 1.0
		active_drone.global_position = drop_pos

	is_driving_drone = !is_driving_drone
	if is_driving_drone:
		camera.current = false
		active_drone.camera.current = true
		active_drone.is_active = true
	else:
		camera.current = true
		active_drone.camera.current = false
		active_drone.is_active = false
		
func show_instruction(text_to_show):
	instruction_text.text = text_to_show
